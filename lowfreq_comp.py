...
__title__ = "Low frequency component analysis and forced pattern filtering"
__reference__ = "Wills et al. 2015, Wills et al. 2020"
...
def peigs(a, rmax):
    ...
    if rmax<np.min((m,n))/10.:
        (d,v) = sp.sparse.linalg.eigs(a,rmax)
    else:
        (d,v) = np.linalg.eig(a)

    if d.size>np.max(d.shape):
        d = np.diag(d)
    ...
    d_min = np.max(d)*np.max((m,n))*np.spacing(1)
    r = np.sum(d>d_min)
